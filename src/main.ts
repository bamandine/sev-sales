...
import { setdbPath, executeQuery, executeMany, executeScript } from "sqlite-electron";
...
const createWindow = () => {
  // Init the database
  setdbPath(path.join(__dirname, 'my-charlott-sales-database.db'));
...
ipcMain.handle("setDatabase", async (event, dbPath) => {
  try {
